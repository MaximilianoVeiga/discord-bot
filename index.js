...
client.on('message', async message => {
    ...
    const command = message.content.includes(PREFIX) ? message.content.split(PREFIX)[1] : !message.content.includes(PREFIX) ? message.content : "";
...
async function collectReaction(message, emojiList, code) {
  const filter = (reaction, user) => {
    return (emojiList.find((item) => item.key === reaction.emoji.name).key != null && user.id === game.userId)
  }

  const newMessage = await message.awaitReactions(filter, {
    max: 1,
    time: 30000,
    errors: ["time"],
